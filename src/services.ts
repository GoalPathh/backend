...
import { CoachRepository, DashboardRepository, GoalRepository, MilestoneRepository, PersonaRepository, UserRepository } from "./repositories.js";
...
export class PersonaService { constructor(private repo=new PersonaRepository()){} async compute(u:string,windowDays:number,force=false){ if (!force){const cached=await this.repo.getCached(u,windowDays,60*60*1000); if (cached){const traits=(cached as any).traits; const evidence=(cached as any).evidence; return {archetype:(cached as any).archetype,headline:"",traits,evidence,advice:deriveAdvice((cached as any).archetype, traits, evidence),generatedAt:(cached as any).computed_at,windowDays:(cached as any).window_days}; } } return this.repo.compute(u,windowDays);} getCoachContext(u:string,windowDays=14){return this.repo.getCoachContext(u,windowDays);} }
